...
- `map_notes` は配布実績とは独立（交通誘導員・配布候補地などの目印）。
...
flowchart TD
  ...
  Login["/login<br/>マジックリンク"] -->|"認証成功"| List

  List["/ 一覧タブ<br/>登録・編集・削除・履歴・コピー"]
  Map["/map 地図タブ<br/>ピン・メモ・配置"]
...
| 画面 | パス | 主な機能 |
|------|------|----------|
| ログイン | `/login` | マジックリンク（デモ環境はワンクリックログイン） |
| 一覧 | `/` | 登録・インライン編集・論理削除/復元・フィルタ・変更履歴・履歴コピー |
| 地図 | `/map` | ピン表示（色分け）・ホバーで履歴・クリックで一覧・配置・住所検索・マップメモ |
